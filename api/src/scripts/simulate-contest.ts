...
function botHandle(i: number): string {
  return `${BOT_HANDLE_PREFIX}${String(i).padStart(4, '0')}`;
}
...
async function registerOrLoginBot(i: number): Promise<Session> {
  const handle = botHandle(i);
  const email = `${handle}@codearena.dev`;

  let res = await apiRequest('POST', '/api/auth/register', { body: { handle, email, password: BOT_PASSWORD } });
  if (res.status === 201 && res.setCookie) {
    return { handle, cookie: extractCookiePair(res.setCookie) };
  }
  if (res.status === 409) {
    res = await apiRequest('POST', '/api/auth/login', { body: { handle, password: BOT_PASSWORD } });
...
async function createBots(): Promise<Session[]> {
  const sessions: Session[] = [];
  for (let i = 1; i <= BOT_COUNT; i++) {
    const session = await registerOrLoginBot(i);
...
async function cleanup(): Promise<void> {
  ...
  const bots = await User.find({ handle: { $regex: `^${BOT_HANDLE_PREFIX}` } })
    .select('_id handle')
    .lean();
...
async function main(): Promise<void> {
  ...
  const sessions = await createBots();
